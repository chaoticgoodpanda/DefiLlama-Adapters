...
// token list
const tokenListsApiEndpoint =
  "https://token-list.solv.finance/vouchers-prod.json";
...
async function tvl(timestamp, block, chainBlocks, network, chainId, transform) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    tokenPairs.push([token.address, token.pool]);
  }
...
async function tokenList(chainId) {
  ...
  for (let token of allTokens) {
    if (chainId == token.chainId) {
      if (
        token.extensions.voucher.underlyingToken &&
        token.extensions.voucher.underlyingToken.symbol != "SOLV"
      ) {
        tokens.push({
...
// node test.js projects/solv-protocol/-old.js
module.exports = {
  ethereum: {
    tvl: ethereumTVL,
