...
const poolWithTokens = [
  ...
  // BALANCER
  [
    "0x58378f5F8Ca85144ebD8e1E5e2ad95B02D29d2BB",
    [
      "0x903bEF1736CDdf2A537176cf3C64579C3867A881",
...
async function getVaultTvl(balances, vaults, tokenAtIndex, block) {
  ...
  for (let i = 0; i < vaults.length; i++) {
    ...
    const bals = [totalAmounts[i].output[0], totalAmounts[i].output[1]];
    ...
    assetBalances.forEach((p) => {
      ...
      if (token === "0xdb0f18081b505a7de20b18ac41856bcb4ba86a1a") {
        sdk.util.sumSingleBalance(
          balances,
          ["wing-finance"],
          BigNumber(balance).div(1e9).toFixed(0)
        );
        return;
      }
...
module.exports = {
  methodology:
    "Tokens deposited to mint oneTokens, Angel and HODL vaults excluding oneTokens",
...
}; // node test.js projects/ichifarm/-old.js
