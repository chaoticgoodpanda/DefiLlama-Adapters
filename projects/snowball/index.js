...
async function getSnowglobeBalances(balances, snowglobes, block) {
  const singleSidedPairs = {
    calls: snowglobes
      .filter((globe) => !globe.pair.includes("-"))
      .map((globe) => ({
  ...
  await Promise.all(
    tokens.output.map((token, idx) => {
      sdk.util.sumSingleBalance(
        balances,
        `avax:${token.output}`,
        tokenBalances.output[idx].output
      );
    })
  );
  ...
  for (let i = 0; i < tokenNames.output.length; i++) {
    if (
      isLP(
        tokenNames.output[i].output,
        tokenNames.output[i].input.target,
        "avax"
      )
    ) {
      lpPositions.push({
      ...
      sdk.util.sumSingleBalance(
        ...
        (lpTokenBalances.output[i].output / 10 ** 12).toFixed(0)
      );
    }
  }
...
async function tvl(_timestamp, _ethereumBlock, chainBlocks) {
  ...
  const deprecatedSnowglobes = data.DeprecatedContracts.filter(
    (contract) => contract.kind === "Snowglobe"
  ).map((contract) => ({
    pair: contract.pair,
    snowglobeAddress: contract.contractAddresses[0],
  }));
  const deprecatedStablevaults = data.DeprecatedContracts.filter(
    (contract) => contract.kind === "Stablevault"
  ).map((contract) => ({ swapAddress: contract.contractAddresses[2] }));

  await getStableVaultBalances(
    balances,
    data.StablevaultContracts.concat(deprecatedStablevaults),
    block
  );
  await getSnowglobeBalances(
    balances,
    data.SnowglobeContracts.concat(deprecatedSnowglobes),
    block
  );
...
}; // node test.js projects/snowball/-old.js
