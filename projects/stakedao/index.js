...
const vaultsPolygon = [crv_3crv_vault_polygon, crv_btc_vault_polygon];

const vaultsAvalanche = [crv_3crv_vault_avalanche];

const vaultsHarmony = [crv_3crv_vault_harmony];

const vaultsBsc = [btcEPS_vault_bsc, EPS3_vault_bsc, fusdt3EPS_vault_bsc];
...
async function ethereum(timestamp, block) {
  ...
  sdk.util.sumSingleBalance(
    balances,
    crvToken,
    (await crvInPerpetual).output.amount
  );
  sdk.util.sumSingleBalance(
    balances,
    fxsToken,
    (await fxsInLocker).output.amount
  );
  sdk.util.sumSingleBalance(
    balances,
    angleToken,
    (await angleInLocker).output.amount
  );
  sdk.util.sumSingleBalance(
    balances,
    angle_vault.usdcToken,
    ((sanUsdcEurVaultBalance * sanUsdcEurRate) / 10 ** 18).toFixed(0)
  );
  return balances;
}
...
async function avax(timestamp, ethBlock, chainBlocks) {
  ...
  await Promise.all(
    vaultsAvalanche.map(async (vault) => {
      const crvBalance = await sdk.api.abi.call({
        target: vault.contract,
        block,
        abi: abi[vault.abi],
  ...
  const dai_eth_address = transformAddress(
    "0xbA7dEebBFC5fA1100Fb055a87773e1E99Cd3507a"
  );
  const usdc_eth_address = transformAddress(
    "0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664"
  );
  const usdt_eth_address = transformAddress(
    ...
    vaultsBsc.map(async (vault) => {
      ...
      switch (vault.crvToken) {
        case "0x2a435ecb3fcc0e316492dc1cdd62d0f189be5640":
          balances["bitcoin"] = crvBalance / 10 ** 18;
          break;
        case "0xaf4de8e872131ae328ce21d909c74705d3aaf452":
          balances["usd-coin"] = crvBalance / 10 ** 18;
          break;
        case "0x373410a99b64b089dfe16f1088526d399252dace":
          balances["tether"] = crvBalance / 10 ** 18;
          break;
      }
    })
  );
...
// node test.js projects/stakedao/-old.js
module.exports = {
  ethereum: {
    tvl: ethereum,
    staking,
