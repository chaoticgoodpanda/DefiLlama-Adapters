...
const fixedValueStakingContracts = [
...
const stakingContractsV1 = fixedValueStakingContracts
  .concat(variableValueStakingContracts)
  .concat(singleSideStakingContracts);
const bankStakingContractsV2 = [
...
async function getPrices(marketDictionary, orderedAssets) {
  ...
  for (const assetName of orderedAssets) {
    let response = await lookupApplications(
      marketDictionary[assetName]["oracleAppId"]
    );
...
function getMarketSupply(assetName, appGlobalState, prices, appDictionary) {
  let underlyingCash =
    assetName === "STBL" || assetName === "vALGO"
      ? appGlobalState[marketStrings.active_collateral]
      : appGlobalState[marketStrings.underlying_cash];
  let supplyUnderlying =
    underlyingCash - appGlobalState[marketStrings.underlying_reserves];
...
async function stakingV1() {
  ...
  for (const contractName of variableValueStakingContracts) {
    ...
    lpCirculations[contractName] =
      contractState[marketStrings.lp_circulation] / 1000000;
  }

  let poolSnapshots = await get(
    "https://thf1cmidt1.execute-api.us-east-2.amazonaws.com/Prod/amm_pool_snapshots/?network=MAINNET"
  );
  let assetSnapshots = await get(
    "https://thf1cmidt1.execute-api.us-east-2.amazonaws.com/Prod/amm_asset_snapshots/?network=MAINNET"
  );
  ...
  for (const poolSnapshot of poolSnapshots.pool_snapshots) {
    for (const contractName of variableValueStakingContracts) {
      if (
        ...
        prices[contractName] =
          poolSnapshot.balance_info.total_usd / lpCirculations[contractName];
      }
    }
  }
  ...
  for (const assetSnapshot of assetSnapshots.asset_snapshots) {
    for (const contractName of singleSideStakingContracts) {
      if (
        assetSnapshot.id ==
        appDictionary["STAKING_CONTRACTS"][contractName]["assetId"]
      ) {
  ...
  for (const contractName of stakingContractsV1) {
    let appGlobalState = await getAppGlobalState(
      appDictionary["STAKING_CONTRACTS"][contractName]["appId"]
    );
    staked += getMarketSupply(
      contractName,
      appGlobalState,
      prices,
      appDictionary["STAKING_CONTRACTS"]
    );
  }
...
async function stakingV2() {
  ...
  for (const contractName of bankStakingContractsV2) {
    let stakingAppGlobalState = await getAppGlobalState(
      appDictionary["STAKING_CONTRACTS"][contractName]["appId"]
    );
    let marketAppGlobalState = await getAppGlobalState(
      appDictionary["STAKING_CONTRACTS"][contractName]["marketAppId"]
    );

    let bAssetStaked =
      stakingAppGlobalState[stakingV2Strings.total_staked] / 1000000;
    let bAssetToUnderlyingExchange =
      marketAppGlobalState[
        marketV2Strings.b_asset_to_underlying_exchange_rate
      ] / 1000000000;
    let underlyingStaked = bAssetStaked * bAssetToUnderlyingExchange;

    let oracleState = await getAppGlobalState(
      marketAppGlobalState[marketStrings.oracle_app_id]
    );

    let oraclePriceFieldName =
      appDictionary["STAKING_CONTRACTS"][contractName]["oracleFieldName"];
...
async function dex() {
  const response = (
    await retry(
      async (bail) =>
        await axios.get(
          "https://thf1cmidt1.execute-api.us-east-2.amazonaws.com/Prod/amm_protocol_snapshot/?network=MAINNET"
        )
    )
...
// node test.js projects/algofi/-old.js
