...
// node test.js projects/hop/-old.js
...
function chainTvl(chain) {
  return async (timestamp, ethBlock, chainBlocks) => {
    ...
    const tokens = await axios(
      "https://raw.githubusercontent.com/hop-protocol/hop/develop/packages/core/build/addresses/mainnet.json"
    );
    for (const tokenConstants of Object.values(tokens.data.bridges)) {
      const chainConstants =
        chain == "xdai" ? tokenConstants["gnosis"] : tokenConstants[chain];
      if (chainConstants === undefined) {
        continue;
      }

      let token =
        chainConstants.l2CanonicalToken ?? chainConstants.l1CanonicalToken;
      ...
      for (const bonder of Object.entries(tokens.data.bonders)) {
        ...
        for (const contract of contractList) {
          if (tokenName === "ETH") {
            ...
            const token =
              tokens.data.bridges[tokenName].ethereum.l1CanonicalToken;
