...
files.forEach((i) => {
  ...
  try {
    ...
    fPath = path.join(projectsFolder, i.name, "-old.js");
...
function getModule(fPath, projectName) {
  ...
  if (typeof module.tvl === "function") {
    const chainsWithTVL = Object.keys(module).filter(
      (chain) =>
        typeof module[chain] === "object" &&
        typeof module[chain].tvl === "function"
    );
    if (chainsWithTVL.length)
      console.log("I am confused:", projectName, chainsWithTVL);
    else module.ethereum = { tvl: module.tvl };
    delete module.tvl;
  }

  Object.keys(module)
    .filter((k) => typeof module[k] !== "object")
    .forEach((i) => {
  ...
  Object.keys(module).forEach((chain) => {
    Object.keys(module[chain]).forEach((key) =>
      addKey(key, `${projectName}-${chain}`)
    );
  });
  return module;
}
...
rModules.forEach((module, i) => {
  if (!Object.keys(module).length)
    return projectMissingChainNames.push(projectNames[i]);
  Object.keys(module).forEach((chain) => {
...
const chainCountTable = Object.keys(chainCount)
  .map((chain) => [chain, chainCount[chain].length, chainCount[chain]])
  .sort((a, b) => b[1] - a[1]);
const keyCountTable = Object.keys(keysCount)
  .map((key) => [key, keysCount[key].length, keysCount[key]])
  .sort((a, b) => b[1] - a[1]);
...
fs.writeFileSync(
  path.join(__dirname, "chains.json"),
  JSON.stringify(chainNames, null, 2)
);
