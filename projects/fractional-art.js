...
async function retrieveVaultsAPI() {
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    vaults_i = vaults_i.data.data.map(
      ({
        analytics,
        pools,
        contractAddress,
...
function getVaultsTvlApi(vaults) {
  return vaults.reduce(
    (acc, vault) =>
      acc.plus(BigNumber(vault.analytics ? vault.analytics.tvlUsd : 0)),
    BigNumber(0)
  );
}
...
// 3. COULD USE core/-old.js functions to get TL locked in given Uniswap pools, but too much to copy-paste from ./CORE/INDEX.JS
/*
const {getUniswapPairInfo, getPairUnderlyingReserves, flattenUnderlyingReserves} = require('./core/-old.js');
